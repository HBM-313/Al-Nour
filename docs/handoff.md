...
**Status (2026-07-22, session 11 — Leverance 1.2: atomisk `record_progress`-RPC, delvist gennemført, IKKE endnu pushet med denne note).**
Se `supabase/migrations/README.md` → "record_progress-RPC + event-idempotens" for fuld beskrivelse. Kort: `progress.ts`s læs-derefter-skriv er erstattet af én atomisk RPC med event-baseret idempotens (`progress_events`-tabel, fail-closed RLS uden policies). Bevist med 11-punkts rollback-markør-regressionstest mod live-DB. Frontend porteret uden signaturændringer. Build-kæde grøn (tsc/oxlint/84 tests/build). **Leverance 1.1 (IndexedDB-kø oven på denne RPC) er sessionens næste skridt — denne note opdateres igen når hele 1.1+1.2 er i mål.**

**Dokumentations-fund (flag til ejer, ikke rettet her):** denne fils historik stopper ved 2026-07-17 (TTS-status). Sessionerne 6–10 (test-framework, ErrorBoundary/fejllogning, CI/CD m.m. — se Claude-projektmappens handoff.md for fuld status) er tilsyneladende aldrig blevet committet til `docs/handoff.md` i selve repoet, kun til Project-filen. De to filer er dermed drevet fra hinanden. Bør reconcileres i en kommende session.
